...
Network = () ->
  ...
  nodeColors = (x) ->
    group_color = '#1f77b4' if x.indexOf('persona')>=0
  ...
  filterNodestoLinks = (curLinks, allNodes) ->
    ...
    console.log(curLinks[0]['source']['id'])
    filterNodes=allNodes.filter (n) ->
      ##curLinks.forEach (l) ->
      ##  console.log(n.id+":"+l.source.id + "->"+l.target.id)
      #(n.id != l.source.id) and (n.id != l.target.id)
  ...
  #TODO no se deben agregar nodos ya presentes en grafo... se debe discriminar por id
  loadJson = (d,i) ->
  ...
  showDetails = (d,i) ->
    ...
    content += '<a href="#" onclick="$(\'div.tooltip\').hide()">x</a></h3>'
    content += '<p class="perfil-details">' + d.shortBio + '   '
    content += '<a href="' + d.url + '">Ver Perfil ▶</a></p>'
...
$ ->
  ...
  d3.selectAll("#groups a").on "click", (d) ->
    newGroup = d3.select(this).attr("id")
    activate("groups", newGroup)
