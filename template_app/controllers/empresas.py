...
def conexiones():
    ...
    alias=alias.decode('utf-8').replace('_',' ').replace('-','.').replace('yy','&')
    organizacion=db.Organizacion(alias=alias,tipoOrg=2)
...
def personasrelacionadas():
    ...
    organizacion=db.Organizacion(alias=alias)
    if organizacion:
        _id=organizacion.id
    else:
...
def mapa_relaciones():
    ...
    org=None
    imagen=IMG(_alt=alias, _src=URL('static','icono-empresas.png'),_height=44)
    org=db.Organizacion(alias=alias,is_active=True,tipoOrg=2)
    ...
    return dict(_id=_id,org=org,imagen=imagen,url=url,entity='empresa')
