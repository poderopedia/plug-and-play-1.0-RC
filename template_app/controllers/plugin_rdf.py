...
class Rdf(object):
    ...
    def read(self, table, record):
        ...
        descriptionNode = { '_rdf:about': self.url( args= [ 'read', str(table), str(record) ] ),
                            'name':'rdf:Description', 'children': [ { 'name': 'rdf:type',
                                                                      '_rdf:resource': table.rdf['type'] } ] }
...
def index():
    rows = [A('exposed tables',_href=URL(r=request,f='ld',args='tables'))]
    for table in db.tables:
        if hasattr(db[table],'rdf'):
            rows.append(A('select %s' % table, _href=URL(r=request,f='ld',args=['select',table])))
